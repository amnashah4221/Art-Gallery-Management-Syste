...
availability = ["Available", "Not for Sale", "Sold", "On Loan"]
...
# Randomly select 60 values from the customized lists
artwork_records = list(zip(
    random.sample(art_ids, 60),
    random.choices(artist_ids, k=60),
    random.sample(titles, 30),
    random.choices(mediums, k=60),
    random.choices(dimensions, k=60),
    random.choices(dates_completed, k=60),
    random.choices(prices, k=60),
    random.choices(availability, k=60)
))

# Insert records into the Artwork table
for record in artwork_records:
    # Ensure that the price is set to None if availability is 'Not for Sale' or 'On Loan'
    if record[7] in ['Not for Sale']:
        price = None
    else:
        price = record[6]
