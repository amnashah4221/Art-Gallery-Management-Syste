...
# Fetch ArtID values and their corresponding prices from Artwork table for artworks that are on loan
cursor.execute('SELECT ArtID, Price FROM Artwork WHERE Availability = \'On Loan\'')
...
# Ensure unique LoanID values by shuffling and slicing the list
random.shuffle(loan_ids)
unique_loan_ids = loan_ids[:60]

# Ensure unique ArtID values by shuffling and slicing the list
random.shuffle(art_ids)
unique_art_ids = art_ids[:60]

# Randomly select values from the customized lists
loan_records = []
for loan_id, art_id in zip(unique_loan_ids, unique_art_ids):
    user_id = random.choice(user_ids)
    loan_amount = art_prices[art_id] - 20000
