CREATE DATABASE ArtGalleryDatabaseManagementSystem
use ArtGalleryDatabaseManagementSystem
GO
...
SELECT * FROM Users WHERE UserEmail = 'user101@example.com';
...
go
CREATE PROCEDURE UpdateLoanDetails
    @LoanID INT,
...
BEGIN
    ...
    BEGIN
        RAISERROR('LoanID %d does not exist.', 16, 1, @LoanID);
        RETURN;
    END
    ...
    UPDATE Loan
    SET
        LoanAmount = COALESCE(@LoanAmount, LoanAmount),
        DurationInDays = COALESCE(@DurationInDays, DurationInDays),
        ExpiryDate = COALESCE(@ExpiryDate, ExpiryDate),
        TransportTracking = COALESCE(@TransportTracking, TransportTracking),
        MonitoringStatus = COALESCE(@MonitoringStatus, MonitoringStatus)
    WHERE
        LoanID = @LoanID;
END;

EXEC UpdateLoanDetails
    @LoanID = 68,
    @LoanAmount = 20000.00,
    @DurationInDays = 30,
    @ExpiryDate = '2024-11-10',
    @TransportTracking = 'FedEx',
...
EXEC sp_SalesReportByDateRange @StartDate = '2024-02-01', @EndDate = '2024-12-01';
GO
...
select * from Loan
EXEC sp_LoanReportByUser @UserID = 3;
GO
...
select * from Gallery
EXEC sp_ExhibitionReportByGallery @GalleryID = 1;
GO

SELECT E.EName, E.ArtID, E.GID, A.Title
FROM Exhibition E
...
CREATE PROCEDURE DeleteArtkByID
    @ArtID INT
AS
BEGIN
...
EXEC DeleteUserByID @UserID = 1;
EXEC DeleteArtworkByID @ArtID = 2;
...
AS
BEGIN
    DELETE FROM Artist WHERE AID = @AID;
END;
GO
EXEC DeleteArtistByID @AID = 71;


go
CREATE PROCEDURE UpdateGalleryDetails
    @GID INT,
    @GalleryName VARCHAR(255),
    @Status VARCHAR(50),
    @Location VARCHAR(255)
...
AS
BEGIN
    INSERT INTO Users (UserEmail, Username, Password, Role, FirstName, LastName, Address, PhoneNo)
    VALUES (@UserEmail, @Username, @Password, @Role, @FirstName, @LastName, @Address, @PhoneNo);
END;
GO
...
AS
BEGIN
    INSERT INTO Artwork (AID, Title, Medium, Dimensions, DateCompleted, Availability, Price)
    VALUES (@AID, @Title, @Medium, @Dimensions, @DateCompleted, @Availability, @Price);
END;
GO
...
AS
BEGIN
    INSERT INTO Gallery (UserID, GalleryName, Status, Location)
    VALUES (@UserID, @GalleryName, @Status, @Location);
END;
GO
